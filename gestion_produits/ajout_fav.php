<?php
    ...
    if (isset($_GET["id"])){
        ...
        if (!isset(mysqli_fetch_assoc(mysqli_query($link, "SELECT product_id FROM favorites WHERE product_id=".$_GET["id"]))["product_id"])){
        ...
        if (!isset(mysqli_fetch_assoc(mysqli_query($link, "SELECT product_id FROM favorites WHERE product_id='".$_POST["product_id"]."'"))["product_id"])){
            //l'objet n'est pas déjà en fav -> on l'ajoute
